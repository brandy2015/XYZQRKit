...
open class LBXScanViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    open func setNeedCodeImage(needCodeImg:Bool){isNeedCodeImage = needCodeImg;}
    //设置框内识别
    ...
    open func handleCodeResult(arrayResult:[LBXScanResult]){
        if let delegate = scanResultDelegate  {
            ...
            let result:LBXScanResult = arrayResult[0]
            showMsgX(title: result.strBarCodeType, message: result.strScanned)
        }
    }
    ...
    public func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]){
        ...
        showMsgX(title: nil, message: NSLocalizedString("Identify failed", comment: "Identify failed"))
    }
    
    private func showMsgX(title:String?,message:String?){
