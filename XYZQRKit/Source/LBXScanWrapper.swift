...
open class LBXScanWrapper: NSObject,AVCaptureMetadataOutputObjectsDelegate,AVCapturePhotoCaptureDelegate {
    ...
    init( videoPreView:UIView,objType:[AVMetadataObject.ObjectType] = [(AVMetadataObject.ObjectType.qr as NSString) as AVMetadataObject.ObjectType],isCaptureImg:Bool,cropRect:CGRect=CGRect.zero,success:@escaping ( ([LBXScanResult]) -> Void) ){
        ...
        isNeedCaptureImage = false//qrFilter//isCaptureImg
        stillImageOutput = AVCaptureStillImageOutput();
        
        super.init()
        if device == nil || input == nil {return}
        if session.canAddInput(input!) {session.addInput(input!)}
        if session.canAddOutput(output) {session.addOutput(output)}
        if session.canAddOutput(stillImageOutput!) {session.addOutput(stillImageOutput!)}
        
        let outputSettings:Dictionary = [AVVideoCodecType.jpeg.rawValue:AVVideoCodecKey]
        stillImageOutput?.outputSettings = outputSettings
        session.sessionPreset = AVCaptureSession.Preset.high
        //参数设置
        output.setMetadataObjectsDelegate(self, queue: DispatchQueue.main)
        output.metadataObjectTypes = objType
        if !cropRect.equalTo(CGRect.zero){//启动相机后，直接修改该参数无效
        ...
        if ( device!.isFocusPointOfInterestSupported && device!.isFocusModeSupported(AVCaptureDevice.FocusMode.continuousAutoFocus) ){
            ...
            }  catch let error as NSError {print("device.lockForConfiguration(): \(error)")}
        }
    }
    ...
    open func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputMetadataObjects metadataObjects: [Any]!, from connection: AVCaptureConnection!) {
        ...
        for current:Any in metadataObjects{
            if (current as AnyObject).isKind(of: AVMetadataMachineReadableCodeObject.self){
                ...
                //码类型
                let codeType = code.type //                print("code type:%@",codeType)
                //码内容
        ...
        if arrayResult.count > 0{
            if isNeedCaptureImage{captureImage()}else{
                stop()
...
//    pragma clang diagnostic pop
        ...
        for connection:AnyObject in connections{
            ...
            for port:Any in connectionTmp.inputPorts{
                if (port as AnyObject).isKind(of: AVCaptureInput.Port.self){
                    ...
                    if portTmp.mediaType == mediaType{return connectionTmp}
                }
            }
        }
        return nil
    ...
    //MARK: 切换识别码的类型  //待测试中途修改是否有效
    open func changeScanType(objType:[AVMetadataObject.ObjectType]){output.metadataObjectTypes = objType}
    open func isGetFlash()->Bool{ if (device != nil &&  device!.hasFlash && device!.hasTorch){return true}else{return false}}
    /** 打开或关闭闪关灯
    ...
    open func changeTorch(){
        if isGetFlash(){
            do{ try input?.device.lockForConfiguration()
                var torch = false
                if input?.device.torchMode == .on       {torch = false}
                else if input?.device.torchMode == .off {torch = true}
    ...
    static func isSysIos8Later()->Bool{return true}
    /**
     识别二维码码图像
    ...
    static func getConcreteCodeImage(srcCodeImage:UIImage,codeResult:LBXScanResult)->UIImage?{
        ...
        guard let imgx = img else {return nil}
        let imgRotation = imageRotation(image: imgx, orientation: UIImage.Orientation.right)
        return imgRotation
    }
    //根据二维码的区域截取二维码区域图像
    static public func getConcreteCodeImage(srcCodeImage:UIImage,rect:CGRect)->UIImage?{
        guard !rect.isEmpty ,let img = imageByCroppingWithStyle(srcImg: srcCodeImage, rect: rect) else {return nil}
    ...
    static public func getConcreteCodeRectFromImage(srcCodeImage:UIImage,codeResult:LBXScanResult)->CGRect{
        if (codeResult.arrayCorner == nil || (codeResult.arrayCorner?.count)! < 4  ){return CGRect.zero}
        let corner:[[String:Float]] = codeResult.arrayCorner  as! [[String:Float]]
        let dicTopLeft     = corner[0];let dicTopRight    = corner[1];let dicBottomRight = corner[2];let dicBottomLeft  = corner[3]
        
        let xLeftTopRatio:Float = dicTopLeft["X"]!          ;       let yLeftTopRatio:Float  = dicTopLeft["Y"]!
        let xRightTopRatio:Float = dicTopRight["X"]!        ;       let yRightTopRatio:Float = dicTopRight["Y"]!
        let xBottomRightRatio:Float = dicBottomRight["X"]!  ;       let yBottomRightRatio:Float = dicBottomRight["Y"]!
        let xLeftBottomRatio:Float = dicBottomLeft["X"]!    ;       let yLeftBottomRatio:Float = dicBottomLeft["Y"]!
