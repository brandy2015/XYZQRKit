...
open class LBXScanView: UIView{
    var viewStyle:LBXScanViewStyle = LBXScanViewStyle() //扫码区域各种参数
    var scanRetangleRect:CGRect = .zero                 //扫码区域
    var scanLineAnimation:LBXScanLineAnimation?         //线条扫码动画封装
    var scanNetAnimation:LBXScanNetAnimation?           //网格扫码动画封装
    var scanLineStill:UIImageView?                      //线条在中间位置，不移动
    var activityView:UIActivityIndicatorView?           //启动相机时 菊花等待
    var labelReadying:UILabel?                          //启动相机中的提示文字
    var isAnimationing:Bool = false                     //记录动画状态
    /**
    初始化扫描界面
    ...
    */
    public init(frame:CGRect, vstyle:LBXScanViewStyle ){viewStyle = vstyle
        switch (viewStyle.anmiationStyle){
            case .LineMove  : scanLineAnimation = LBXScanLineAnimation.instance()
            case .NetGrid   : scanNetAnimation  = LBXScanNetAnimation.instance()
            case .LineStill : scanLineStill     = UIImageView(image: viewStyle.animationImage)
            default:break
        }
        var frameTmp = frame;
    ...
    override init(frame: CGRect) {
        var frameTmp = frame;
        frameTmp.origin = .zero
        super.init(frame: frameTmp)
        backgroundColor = UIColor.clear
    }
    required public init?(coder aDecoder: NSCoder){self.init()}
    deinit{scanLineAnimation?.stopStepAnimating();scanNetAnimation?.stopStepAnimating();print("LBXScanView deinit")}
    
    
    /**
    *  开始扫描动画
    */
    func startScanAnimation(){if isAnimationing{return};isAnimationing = true
        ...
        switch viewStyle.anmiationStyle{
            ...
            case .LineStill:
                let stillRect = CGRect(x: cropRect.origin.x+20,y: cropRect.origin.y + cropRect.size.height/2,width: cropRect.size.width-40,height: 2);
    ...
    /**开始扫描动画**/
    func stopScanAnimation(){isAnimationing = false
        switch viewStyle.anmiationStyle{
            case .LineMove  :scanLineAnimation?.stopStepAnimating()
            case .NetGrid   :scanNetAnimation?.stopStepAnimating()
            case .LineStill :self.scanLineStill?.isHidden = true
            default: break
        }
    }
    // Only override drawRect: if you perform custom drawing.
    // An empty implementation adversely affects performance during animation.
    override open func draw(_ rect: CGRect){drawScanRect()} // Drawing code
    //MARK:----- 绘制扫码效果-----
    func drawScanRect(){
